...
class UserModel extends Model
{
    ...
    public function findUser($category)
    {
        ...
        if ($category === 'email_id') {
            return $findUser->getOne('users', $category, $this->email_id);
        }

        if ($category === 'password_reset_token') {
            return $findUser->getOne('users', $category, $this->password_reset_token);
        }
    ...
    public function uploadImage($user_id)
    {
        ...
        if (in_array($fileActualExt, $allowed)) {
            if ($fileError === 0) {
                ...
                $query->execute();
                $previous_image_name = $query->fetchAll(PDO::FETCH_ASSOC);
                $filePrevDestination = '/public/img/uploads/' . $previous_image_name[0]['user_image'];
    ...
    public function viewAllTrainsByUsers()
    {
    ...
    { //Ashika
        ...
        $registerSetValueArray['count'] = $this->count;
