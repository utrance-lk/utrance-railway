...
class FreightModel extends Model
{
    ...
    public function getFreightPrice(){
        ...
        //var_dump($this->start_station_id);

        $query = APP::$APP->db->pdo->prepare("SELECT station_id FROM stations WHERE station_name=:dest_station ");
        $query->bindValue(":dest_station", $this->destination);
        $query->execute();
        $this->dest_station_id = $query->fetchAll(PDO::FETCH_ASSOC);
        //var_dump($this->dest_station_id);

        $query = APP::$APP->db->pdo->prepare("SELECT route_id FROM routes WHERE start_station_id=:start_station AND dest_station_id=:dest_station LIMIT 1");
        $query->bindValue(":dest_station",$this->dest_station_id[0]['station_id']);
        $query->bindValue(":start_station",$this->start_station_id[0]['station_id']);
        $query->execute();
        $this->route_id = $query->fetchAll(PDO::FETCH_ASSOC);
        //var_dump($this->route_id);

        $query = APP::$APP->db->pdo->prepare("SELECT metal,agricultural_products,textile_products,timber FROM freight_price WHERE route_id=:route_id");
        $query->bindValue(":route_id",$this->route_id[0]['route_id']);
        $query->execute();
        $this->freight_results= $query->fetchAll(PDO::FETCH_ASSOC);
